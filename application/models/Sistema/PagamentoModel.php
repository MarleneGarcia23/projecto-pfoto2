...
class PagamentoModel extends CI_Model
{
    ...
    {
        if ($this->db->insert('pagamento', $dados)) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    {
        return $this->db->query("SELECT * FROM pagamento")->result();
    }
    //funcao que lista 
    ...
    {
        return $this->db->query("SELECT id as npagamento FROM pagamento ORDER BY id DESC LIMIT 1;")->result();
    }
    //função actualizar
    public function actualizar($id, $dados)
    {
        $this->db->where('id', $id);
        if ($this->db->update('pagamento', $dados)) {
            return TRUE;
        }
        return FALSE;
    }
