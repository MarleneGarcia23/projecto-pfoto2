                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $cont = 1; ?>
                                <?php foreach ($dados["sub_desc"] as $valor) { ?>
                                    <tr>
                                        <td><?= $cont++ ?></td>
                                        <td><?= $valor->designacao; ?></td>
                                        <td><?= strtoupper($valor->tipo); ?></td>
                                        <td><?= $valor->valor; ?></td>
                                        <td><?= $valor->descricao; ?></td>
                                        <td>
                                            <?php if ($this->basehome->getModulo($this->session->userdata('id'), 'CONFIGURACOES') != null) { ?>
                                                <a href="<?= base_url(); ?>sub_desc/editar/<?= $valor->id; ?>" class="btn btn-instagram">Editar</a>
                                                <a onclick="(confirm('Pretende efetuar esta operacão')==true)?location.href='<?= base_url(); ?>sub_desc/eliminar/<?= $valor->id; ?>':''"  class="btn btn-danger">Eliminar</a>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php } ?>
