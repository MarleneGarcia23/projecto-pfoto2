                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $cont = 1; ?>
                                <?php foreach ($dados["salario"] as $valor) { ?>
                                    <tr>
                                        <td><?= $cont++ ?></td>
                                        <td><?= $valor->idsalario; ?></td>
                                        <td><?= $valor->funcionario; ?></td>
                                        <td><?= $valor->mes; ?></td>
                                        <td><?= $valor->total; ?>.00(AKZ)</td>
                                        <td><?= $valor->datasalario; ?></td>
                                        <td>
                                            <button class="btn btn-success btn-sm" onClick="imprimir(<?= $valor->idsalario; ?>);">
                                                <i class="glyphicon glyphicon-print"></i> Imprimir
                                            </button>

                                            <?php if ($this->basehome->getModulo($this->session->userdata('id'), 'CONFIGURACOES') != null) { ?>
                                                <a onclick="(confirm('Pretende efetuar esta operacão')==true)?location.href='<?= base_url(); ?>salario/eliminar/<?= $valor->idsalario; ?>':''"  class="btn btn-danger">Eliminar</a>
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php } ?>
