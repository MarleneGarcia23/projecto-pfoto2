                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $cont = 1; ?>
                                <?php foreach ($dados["facturavenda"] as $valor) { ?>
                                    <tr>
                                        <td><?= $cont++ ?></td>
                                        ...
                                        <td>
                                            <button class="btn btn-success btn-sm" onClick="imprimir_venda(<?= $valor->idfactura; ?>);">
                                                <i class="glyphicon glyphicon-print"></i> Imprimir
                                            </button>
                                            <?php if ($this->basehome->getModulo($this->session->userdata('id'), 'CONFIGURACOES') != null) { ?>
                                                <a onclick="(confirm('Pretende efetuar esta operacão')==true)?location.href='<?= base_url(); ?>factura/eliminarvenda/<?= $valor->idfactura; ?>':''"  class="btn btn-danger">Eliminar</a>                           
