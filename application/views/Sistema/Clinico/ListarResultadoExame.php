                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                <?php $cont = 1; ?>
                                <?php foreach ($dados["resultadoexame"] as $valor) { ?>
                                    <tr>
                                        <td><?= $cont++ ?></td>
                                        ...
                                        <td><?= date('d/m/Y H:m:s', strtotime($valor->dataresultadoexame)); ?></td>
                                        <td>
                                            ...
                                            <a class="btn btn-primary" href="<?= base_url() ?>clinico/editarresultadoexame/<?= $valor->idpedido ?>"><i class="glyphicon glyphicon-edit"></i> Editar Resultado</a>
                                            <?php if ($this->basehome->getModulo($this->session->userdata('id'), 'CONFIGURACOES') != null) { ?>
												<a onclick="(confirm('Pretende efetuar esta operacão')==true)?location.href='<?= base_url(); ?>clinico/eliminarresultadoexame/<?= $valor->idpedido; ?>':''"  class="btn btn-danger">Eliminar</a>                           
                                            <?php } ?>
                                        </tr>
                                <?php } ?>
                            </tbody>
